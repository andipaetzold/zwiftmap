...
} from "./path-to-regex-modified";
import {
  URLPatternResult,
  URLPatternInit,
  URLPatternKeys,
} from "./url-pattern.interfaces";
import {
  DEFAULT_OPTIONS,
  HOSTNAME_OPTIONS,
  PATHNAME_OPTIONS,
...
function extractValues(url: string, baseURL?: string): URLPatternInit {
  ...
  return {
    ...
    pathname: o.pathname,
    search:
      o.search !== "" ? o.search.substring(1, o.search.length) : undefined,
...
function tokensToPattern(
  ...
  for (const token of tokens) {
    ...
    const needsGrouping =
      token.suffix !== "" ||
      (token.prefix !== "" &&
        (token.prefix.length !== 1 || !optionsPrefixes.includes(token.prefix)));
    ...
    const segmentWildcardPattern = `[^${escapeRegexpString(
      options.delimiter || "/#?"
    )}]+?`;
    ...
    try {
      ...
      for (component of COMPONENTS) {
        ...
        try {
          ...
          this.regexp[component] = tokensToRegexp(
            tokens,
            this.keys[component],
            options
          );
