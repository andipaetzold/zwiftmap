      ...
      <SimpleListItem aria-hidden="true">
        <div style={{ width: "100%", height: 100 }}>
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={[surfacesWithDistance]} layout="vertical">
              <XAxis
                type="number"
                allowDecimals={false}
                tickCount={4}
              ...
              <YAxis
                ...
                width={15}
              />
              {SURFACES.filter((s) => surfacesWithDistance[s] > 0).map((s) => (
                <Bar
                  key={s}
                  dataKey={s}
        ...
        <ul style={{ padding: 0, margin: 0 }}>
          {SURFACES.filter((s) => surfacesWithDistance[s] > 0).map((s) => (
            <li key={s} style={{ display: "block" }}>
              ...
              {SURFACE_CONSTANTS[s].label}:{" "}
              <Distance distance={surfacesWithDistance[s]} /> (
              {FORMAT_PERCENT.format(surfacesWithDistance[s] / route.distance)})
            </li>
          ))}
        </ul>
      </SimpleListItem>
